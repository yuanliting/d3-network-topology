...
export const findLinks = (nodeId, links) => {
    ...
    for (let link of links) {
      if (link.source.id === nodeId || link.target.id === nodeId) nodeLinks.push(link)
    }
  ...
  export const removeNode = (nodeId, nodes, cb) => {
    console.log('removeNode', nodeId, nodes)
  ...
  export const rebuildLinks = (nodes, links) => {
    ...
    let removed = []
    for (let link of links) {
      if (nodeExists(link.source.id, nodes) && nodeExists(link.target.id, nodes)) {
        newLinks.push(link)
      } else {
        removed.push(link)
      }
  ...
  const isLinked = (nodeId, links) => {
    let index = links.findIndex(
      (link) => { return (link.target.id === nodeId || link.source.id === nodeId) }
    )
    return (index > -1)
  }
  ...
  // node formatter
  export const newNode = (nodeId, name = newNodeName(), imgType = 1, icon = 'img') => {
    return { id: nodeId, name: name, imgType: imgType, icon: icon }
  }
